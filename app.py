...
from recommendation_engine import get_most_common_customer_combinations, get_combinations_for_item, get_recommendations_for_cart
...
def search_cart_combinations():
    ...
    if not selected_items:
        product_list = get_unique_products()
        error_message = "Bitte wählen Sie mindestens ein Item aus."
        return render_template('recommendation_engine.html',
                               static_combinations=get_most_common_customer_combinations("holzbau.db"),
                               product_list=product_list,
                               error=error_message)

    # Erstelle einen String, um den Warenkorb anzuzeigen
    cart_items_str = ", ".join(selected_items)

    # Empfehlungen basierend auf dem Warenkorb ermitteln
