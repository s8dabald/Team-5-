...
def get_top_combinations():
    try:
        ...
        if df.empty:
            return jsonify({"error": "No order data available"}), 500
        ...
        if frequent_itemsets.empty:
            return jsonify({"error": "Not enough frequent itemsets"}), 500

        rules = association_rules(frequent_itemsets, metric="lift", min_threshold=1)

        if rules.empty:
            return jsonify({"error": "No strong associations found"}), 500
        ...
        return jsonify({"top_combinations": rules[['items', 'percentage']].to_dict(orient='records')})
    except Exception as e:
...
@app.route('/recommendation_engine', methods=['GET'])
def get_item_combinations(item):
    try:
        query = """
        SELECT OrderId, Description 
        FROM Order_DB
        ...
        basket = df.groupby(['OrderId', 'Description'])['Description'].count().unstack(fill_value=0)
        basket = basket > 0  # Converts to boolean

        frequent_itemsets = apriori(basket, min_support=0.3, use_colnames=True)
        rules = association_rules(frequent_itemsets, metric="lift", min_threshold=1)

        if rules.empty:
            return jsonify({"error": "No strong associations found for this item"}), 500
        ...
        return jsonify(
            item_rules[['consequents', 'percentage']].head(5).rename(columns={'consequents': 'combination'}).to_dict(
                orient='records'))
    except Exception as e:
        return jsonify({"error": f"Error processing item combinations: {str(e)}"}), 500
